...
void main()
{
	vec4 data = texture(butterflySampler, vec2(uvVar.y, butterflyPass));
	vec2 indices = data.rg;
	vec2 weights = data.ba;

	vec4 input1 = texture(FFTSampler, vec2(uvVar.x, indices.x)).rgba;
	vec4 input2 = texture(FFTSampler, vec2(uvVar.x, indices.y)).rgba;
